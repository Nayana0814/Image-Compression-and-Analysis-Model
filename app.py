...
# -----------------------------
yolo_model = YOLO("yolov8n.pt")  # small version, fast
...
with col1:
    st.image("logo.png", width=180)  # place logo.png in the same folder
with col2:
    st.markdown("<h1 style='text-align: center; color: #4CAF50;'>Compress and Analyse Images</h1>", unsafe_allow_html=True)
    st.markdown(
...
if uploaded_file is not None:
    ...
    # Resize only if larger than 256x256
    width, height = image.size
    if width > 256 or height > 256:
        compressed_img = image.resize((256, 256), Image.LANCZOS)
    else:
        compressed_img = image.copy()

    # Display compressed image
    st.image(compressed_img, caption=f"Compressed Image ({compressed_img.size[0]}x{compressed_img.size[1]})", use_container_width=True)

    # Prepare buffer and choose format
    buf = io.BytesIO()
    if width <= 256 or height <= 256:
